...
object Movie extends App {
  ...
  implicit object ParsableMovie extends Parsable[Movie] {
    /**
      ...
      * @return a Try[Movie]
      */
    def parse(w: String): Try[Movie] = Try(Movie.apply(w.split(",").toSeq)) // TO BE IMPLEMENTED
  }
...
object Rating {
  ...
  def apply(s: String): Rating = (for (ws <- rRating.unapplySeq(s)) yield for (w <- ws) yield Option(w)) match{
    case Some(Seq(Some(value), _, Some(x))) => Rating(value, x.toIntOption)
    case Some(Seq(Some(value), _, _)) => Rating(value, None)
    case _ => throw ParseException(s"Error")
  }
